...
from PyQt5.QtWidgets import (QApplication, QWidget, QPushButton,QLineEdit,
                             ...
                             QTreeWidgetItem, QComboBox, QCheckBox)
...
from OCC.Core.gp import gp_Pnt, gp_Dir, gp_Ax2, gp_Trsf, gp_Vec, gp_Ax1, gp_Quaternion
...
from OCC.Core.Quantity import (Quantity_Color, Quantity_NOC_RED,
                               Quantity_NOC_GREEN, Quantity_NOC_BLUE1)
...
"""在数模中生成箭头"""
def make_arrow(start_point, end_point, shaft_radius=1, head_length=10, head_radius=3):
    # 0. 创建箭杆、箭头，默认为沿z轴生成，如：箭头 hsp = [0,0,0] hep = [0,0,head_length]
    shaft_length = end_point.Distance(start_point) - head_length
    shaft = BRepPrimAPI_MakeCylinder(shaft_radius, shaft_length).Shape()
    head = BRepPrimAPI_MakeCone(head_radius, 0, head_length).Shape()

    # 1. 先将箭头平移至箭杆尾端，平移向量hsp = [0,0,0] ——> [0,0,shaft_length]
    ep = gp_Pnt(0, 0, shaft_length)
    trsf = gp_Trsf()
    trsf.SetTranslation(gp_Vec(gp_Pnt(0, 0, 0), ep))
    head_transformed = BRepBuilderAPI_Transform(head, trsf).Shape()
    full_arrow = BRepAlgoAPI_Fuse(shaft, head_transformed).Shape()   # 将圆柱和锥形头部组合

    # 2. 整体平移至起点start_point
    trsf = gp_Trsf()
    trsf.SetTranslation(gp_Vec(gp_Pnt(0, 0, 0), start_point))
    full_arrow1 = BRepBuilderAPI_Transform(full_arrow, trsf).Shape()

    # 3. 整体旋转，当前箭方向（z轴）旋转到目标方向（start_point，end_point）
    arrow_direction = gp_Dir(end_point.XYZ() - start_point.XYZ())    # 箭头的目标方向
    if arrow_direction.X() == 0 and arrow_direction.Y() == 0:        # 当箭头方向为z轴时，不需要旋转
        return full_arrow1
    trsf1 = gp_Trsf()                                                # 创建变换矩阵
    z_axis = gp_Dir(0, 0, 1)                                         # 当前方向
    angle = z_axis.Angle(arrow_direction)                            # 旋转角度
    rotation_axis = z_axis.Crossed(arrow_direction)                  
    arrow_axis = gp_Ax1(start_point, rotation_axis)                  # 旋转轴，过当前起点，与当前方向、目标方向垂直
    trsf1.SetRotation(arrow_axis, angle)                             # 设置旋转，沿arrow_axis旋转angle角度
    full_arrow_transformed = BRepBuilderAPI_Transform(full_arrow1, trsf1).Shape()
    return full_arrow_transformed

"""在数模展示中显示xyz（细线）"""
def add_axis(display, length = 100):

    # 定义X轴
    x_axis = BRepBuilderAPI_MakeEdge(gp_Pnt(0, 0, 0), gp_Pnt(length, 0, 0)).Shape()
    x_color = Quantity_Color(Quantity_NOC_RED) # , Quantity_TOC_RGB
    display.DisplayColoredShape(x_axis, x_color)

    # 定义Y轴
    y_axis = BRepBuilderAPI_MakeEdge(gp_Pnt(0, 0, 0), gp_Pnt(0, length, 0)).Shape()
    y_color = Quantity_Color(Quantity_NOC_GREEN)
    display.DisplayColoredShape(y_axis, y_color)

    # 定义Z轴
    z_axis = BRepBuilderAPI_MakeEdge(gp_Pnt(0, 0, 0), gp_Pnt(0, 0, length)).Shape()
    z_color = Quantity_Color(Quantity_NOC_BLUE1)
    display.DisplayColoredShape(z_axis, z_color)

class ErrorInputDialog(QDialog):
    def __init__(self, item_text, title = "发生错误"):
        super().__init__()
...
class SetInputDialog(QDialog):
    def __init__(self, match_flag, face_filter, threshold):
        super().__init__()
        self.initUI(match_flag, face_filter, threshold)

    def initUI(self, match_flag, face_filter, threshold):
        self.resize(400, 200)
        self.buttonBox_2 = QDialogButtonBox(self)
        self.buttonBox_2.setGeometry(QtCore.QRect(120, 168, 156, 23))
        ...
        self.widget.setGeometry(QtCore.QRect(10, 10, 377, 148))
        ...
        self.checkBox.setChecked(match_flag)
        ...
        self.retranslateUi(face_filter, threshold)
        QtCore.QMetaObject.connectSlotsByName(self)

    def retranslateUi(self, face_filter, threshold):
        _translate = QtCore.QCoreApplication.translate
        self.setWindowTitle(_translate("Dialog", "面匹配设置"))
        self.label.setText(_translate("Dialog", "开启面匹配："))
        self.checkBox.setText(_translate("Dialog", "是"))
        self.label_2.setText(_translate("Dialog", "匹配面1类型："))
        self.comboBox.setItemText(0, _translate("Dialog", "GeomAbs_Plane"))
        self.comboBox.setItemText(1, _translate("Dialog", "GeomAbs_Cylinder"))
        self.comboBox.setItemText(2, _translate("Dialog", "GeomAbs_Cone"))
        self.comboBox.setItemText(3, _translate("Dialog", "GeomAbs_Sphere"))
        self.comboBox.setItemText(4, _translate("Dialog", "GeomAbs_Torus"))
        self.comboBox.setItemText(5, _translate("Dialog", "GeomAbs_BezierSurface"))
        self.comboBox.setItemText(6, _translate("Dialog", "GeomAbs_BSplineSurface"))
        self.comboBox.setItemText(7, _translate("Dialog", "GeomAbs_SurfaceOfRevolution"))
        self.comboBox.setItemText(8, _translate("Dialog", "GeomAbs_SurfaceOfExtrusion"))
        self.comboBox.setItemText(9, _translate("Dialog", "GeomAbs_OffsetSurface"))
        self.comboBox.setItemText(10, _translate("Dialog", "GeomAbs_OtherSurface"))
        self.comboBox.setCurrentIndex(face_filter[0])
        self.label_3.setText(_translate("Dialog", "匹配面2类型："))
        self.comboBox_2.setItemText(0, _translate("Dialog", "GeomAbs_Plane"))
        self.comboBox_2.setItemText(1, _translate("Dialog", "GeomAbs_Cylinder"))
        self.comboBox_2.setItemText(2, _translate("Dialog", "GeomAbs_Cone"))
        self.comboBox_2.setItemText(3, _translate("Dialog", "GeomAbs_Sphere"))
        self.comboBox_2.setItemText(4, _translate("Dialog", "GeomAbs_Torus"))
        self.comboBox_2.setItemText(5, _translate("Dialog", "GeomAbs_BezierSurface"))
        self.comboBox_2.setItemText(6, _translate("Dialog", "GeomAbs_BSplineSurface"))
        self.comboBox_2.setItemText(7, _translate("Dialog", "GeomAbs_SurfaceOfRevolution"))
        self.comboBox_2.setItemText(8, _translate("Dialog", "GeomAbs_SurfaceOfExtrusion"))
        self.comboBox_2.setItemText(9, _translate("Dialog", "GeomAbs_OffsetSurface"))
        self.comboBox_2.setItemText(10, _translate("Dialog", "GeomAbs_OtherSurface"))
        self.comboBox_2.setCurrentIndex(face_filter[1])
        self.label_4.setText(_translate("Dialog", "面中轴线夹角(°)≤"))
        self.lineEdit.setText(_translate("Dialog", f"{threshold[0]}"))
        self.label_5.setText(_translate("Dialog", "中轴线、中心连线夹角(°)≤"))
        self.lineEdit_2.setText(_translate("Dialog", f"{threshold[1]}"))
        self.label_6.setText(_translate("Dialog", "中心点间距(mm)≤"))
        self.lineEdit_3.setText(_translate("Dialog", f"{threshold[2]}"))
        
    def getTextInput(self):
        match_flag  = self.checkBox.isChecked()
        face_filter = [self.comboBox.currentIndex(), self.comboBox_2.currentIndex()]
        threshold   = [float(self.lineEdit.text()), float(self.lineEdit_2.text()), float(self.lineEdit_3.text())]
        return match_flag, face_filter, threshold
...
class LabelShapeV0(object):
    def __init__(self):
        ...
        self.face_filter  = [GeomAbs_Plane, GeomAbs_Cylinder]
        self.threshold    = [5., 15., 200.]              # 阈值：中轴线夹角(°)、中轴线与中心点连线夹角(°)、中心点距离(mm)
        self.threshold0   = [5., 15., 200.]
        # 面匹配初始化
        self.threshold[:2]   = np.cos(np.radians(self.threshold0[:2])).tolist()
        self.threshold[2]    = self.threshold0[2]
        self.face_info       = list()
        self.face_match_list = list()
    ...
    def setupUi(self, LabelModelV0):
        ...
        self.gridLayout.addWidget(self.LastFile, 0, 1, 1, 1)
        ...
        self.gridLayout.addWidget(self.NextFile, 1, 1, 1, 1)
        ...
        self.gridLayout.addWidget(self.Nothing, 2, 1, 1, 1)
        self.verticalLayout_2.addLayout(self.gridLayout)

        self.SetMatch = QPushButton(self.centralwidget)
        sizePolicy = QSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.SetMatch.sizePolicy().hasHeightForWidth())
        self.SetMatch.setSizePolicy(sizePolicy)
        font = QtGui.QFont()
        font.setFamily("Microsoft YaHei UI")
        font.setPointSize(10)
        self.SetMatch.setFont(font)
        self.SetMatch.setObjectName("Match")
        self.SetMatch.clicked.connect(self.setMatch)
        self.gridLayout.addWidget(self.SetMatch, 0, 2, 1, 1)

        self.Match = QPushButton(self.centralwidget)
        ...
        self.Match.clicked.connect(self.matchFace)
        self.gridLayout.addWidget(self.Match, 1, 2, 1, 1)
        ...
        self.actionda.triggered.connect(self.openFile)
        self.actionopen_file = QAction(LabelModelV0)
        self.actionopen_file.setObjectName("actionopen_file")
        self.actionopen_file.triggered.connect(self.openDir)
        self.actionSet = QAction(LabelModelV0)
        self.actionSet.setObjectName("setMatch")
        self.actionSet.triggered.connect(self.setMatch)
        ...
        Shortcut_set = QShortcut(QKeySequence(Qt.Key_E), self.SetMatch)
        Shortcut_set.activated.connect(self.setMatch)
        Shortcut_set = QShortcut(QKeySequence(Qt.Key_M), self.Match)
        Shortcut_set.activated.connect(self.matchFace)

    def setMatch(self):
        setDialog = SetInputDialog(self.match_flag, self.face_filter, self.threshold0)
        if setDialog.exec_() == QDialog.Accepted:
            self.match_flag, self.face_filter, self.threshold0 = setDialog.getTextInput()
            self.threshold[:2] = np.cos(np.radians(self.threshold0[:2])).tolist()
            self.threshold[2]  = self.threshold0[2]
    ...
    def retranslateUi(self, LabelModelV0):
        ...
        self.SetMatch.setText(_translate("LabelModelV0", "面匹配设置"))
        self.Match.setText(_translate("LabelModelV0", "面匹配"))
        ...
        self.actionda.setText(_translate("LabelModelV0", "打开文件(o)"))
        self.actionopen_file.setText(_translate("LabelModelV0", "打开文件夹(f)"))
        self.actionSet.setText(_translate("LabelModelV0", "面匹配设置(e)"))
    ...
    def getfacename(self, face):
        ...
        if surf_type == GeomAbs_Plane:
            gp_pla = surf.Plane()
            axis = gp_pla.Axis().Direction() # the cylinder axis
            central_point_line = [x, y, z, axis.X(), axis.Y(), axis.Z()]
            return central_point_line, surf_type, "GeomAbs_Plane"
        elif surf_type == GeomAbs_Cylinder:
            gp_cyl = surf.Cylinder()
            axis = gp_cyl.Axis().Direction() # the cylinder axis
            central_point_line = [x, y, z, axis.X(), axis.Y(), axis.Z()]
            return central_point_line, surf_type, "GeomAbs_Cylinder"
        elif surf_type == GeomAbs_Cone:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_Cone"
        elif surf_type == GeomAbs_Sphere:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_Sphere"
        elif surf_type == GeomAbs_Torus:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_Torus"
        elif surf_type == GeomAbs_BezierSurface:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_BezierSurface"
        elif surf_type == GeomAbs_BSplineSurface:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_BSplineSurface"
        elif surf_type == GeomAbs_SurfaceOfRevolution:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_SurfaceOfRevolution"
        elif surf_type == GeomAbs_SurfaceOfExtrusion:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_SurfaceOfExtrusion"
        elif surf_type == GeomAbs_OffsetSurface:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_OffsetSurface"
        elif surf_type == GeomAbs_OtherSurface:
            return [x, y, z] + [0]*3, surf_type, "GeomAbs_OtherSurface"
        else:
    ...
    def openShape(self):
        ...
        isOk, errstr = self.topoChecker(solid)
        if not isOk:
            msg_box = QMessageBox()
            msg_box.setIcon(QMessageBox.Warning)  # 设置图标，可选Information, Warning, Critical, Question
            msg_box.setWindowTitle('警告！')
            msg_box.setText('加载数模失败，不支持或错误的数模！')  # 设置消息框的主要文本
            msg_box.setInformativeText(errstr)  # 可选的，提供更多信息
            # msg_box.setDetailedText(errstr)  # 可选的，展示更多细节
            msg_box.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)  # 设置按钮，可组合使用
            msg_box.setDefaultButton(QMessageBox.Ok)  # 设置默认按钮
            msg_box.exec_()
            # self.widget.QMessageBox.warning(self, "warning", "Fail to load, unsupported or wrong STEP.")
            self.file_name = None
            return
        ...
        # 添加坐标轴
        x = make_arrow(gp_Pnt(0, 0, 0), gp_Pnt(100, 0, 0), shaft_radius=2, head_length=10, head_radius=5)
        y = make_arrow(gp_Pnt(0, 0, 0), gp_Pnt(0, 100, 0), shaft_radius=2, head_length=10, head_radius=5)
        z = make_arrow(gp_Pnt(0, 0, 0), gp_Pnt(0, 0, 100), shaft_radius=2, head_length=10, head_radius=5)
        x_color = Quantity_Color(Quantity_NOC_RED) # , Quantity_TOC_RGB
        y_color = Quantity_Color(Quantity_NOC_GREEN)
        z_color = Quantity_Color(Quantity_NOC_BLUE1)

        add_axis(self.display, 200)
        self.display.DisplayColoredShape(x, x_color)
        self.display.DisplayColoredShape(y, y_color)
        self.display.DisplayColoredShape(z, z_color)
        ...
        for fid, face in enumerate(self.faces_list):
            central_point_line, surf_type, facename = self.getfacename(face)
            self.FaceList.addItem(f"{fid}: {facename}")
            self.face_info += [[fid, surf_type] + central_point_line]
        # 标签列表窗口及数据初始化
    ...
    """
    def getCentral(self, face, surf_type):
        ...
        if surf_type == GeomAbs_Cylinder:
            ...
            axis = gp_cyl.Axis().Direction() # the cylinder axis
            central_point = [location.X(),location.Y(),location.Z()]
            central_line = [axis.X(), axis.Y(), axis.Z()]
        return central_point, central_line
    """

    def matchFace(self):
        face_info = np.array(self.face_info)
        n1 = face_info[:, 1] == self.face_filter[0]
        n2 = face_info[:, 1] == self.face_filter[1]
        fid1 = face_info[n1, 0].tolist()
        fid2 = face_info[n2, 0].tolist()
        if (not fid1) or (not fid2):
            return
        face_norm1 = face_info[n1, 5:]      # /np.linalg.norm(face_info[n1, 5:])
        face_norm2 = face_info[n2, 5:]      # /np.linalg.norm(face_info[n2, 5:])
        face_central1 = face_info[n1, 2:5]
        face_central2 = face_info[n2, 2:5]

        threshold1, threshold2, threshold3 = self.threshold
        indices = np.where(abs(face_norm1 @ face_norm2.T) > threshold1)
        for indix, indiy in zip(indices[0], indices[1]):
            face_central_point1 = face_central1[indix, :]
            face_central_point2 = face_central2[indiy, :]
            face_c2c = face_central_point1 - face_central_point2
            dis12 = np.linalg.norm(face_c2c)
            if dis12 != 0:
                face_c2c_norm = face_c2c/dis12
            else:
                face_c2c_norm = face_norm1[indix, :]
            angle1 = abs(face_norm1[indix, :] @ face_c2c_norm.T)
            angle2 = abs(face_norm2[indiy, :] @ face_c2c_norm.T)
            if dis12 <= threshold3 and angle1 >= threshold2 and angle2 >= threshold2:
                faces_id = [int(fid1[indix]), int(fid2[indiy])]
                self.face_match_list += [faces_id]
                self.addLabel(faces_id[0], 1, self.FaceList.item(faces_id[0]).text())
                self.addLabel(faces_id[1], 2, self.FaceList.item(faces_id[1]).text())
        self.savelabel()
    ...
    def addLabels(self, face_id, sem_label, dialog_text):
        if len(face_id) == 1:
            self.addBotLabel(face_id[0], sem_label, self.FaceList.item(face_id[0]).text())
        else:
    ...
    def info(self):
        ...
        infomation = "\n快捷键：\n    o：打开文件\n    f：打开文件夹\n    c：清除\n    a：上一个\n    d：下一个\n    q：？\n    e：面匹配设置\n    m：面匹配"
        infodialog = ErrorInputDialog(intro0 + intro1 + intro2 + infomation, title = "说明及快捷键")
