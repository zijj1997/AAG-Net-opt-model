...
from OCC.Extend.DataExchange import read_step_file, STEPControl_Reader
...
class MachiningFeature:
    ...
    # shape, labels = self._apply_feature(self.shape, self.labels, self.feat_type, feat_face,feat_dir * depth, bound_max)

    def _apply_hole_feature(self, old_shape, old_labels, feat_type, feat_face, depth_dir, bound_max):
        feature_maker = BRepFeat_MakePrism()
        feature_maker.Init(old_shape, feat_face, TopoDS_Face(), occ_utils.as_occ(depth_dir, gp_Dir), False, False)
        feature_maker.Build()
        ...
        #     if depth_dir[i] != 0  and depth_dir[i]>0:
        #         lenth[i] = 8
        #         lenth2[i] = 28
        #     elif depth_dir[i] != 0  and depth_dir[i]<0:
        #         lenth[i] = -8
        #         lenth2[i] = -28
        #     elif depth_dir[i] == 0:
        #         lenth[i] = 0
        ...
        #         lenth[i] = -0
        #         lenth2[i] = -0
        # feature_maker1 =  BRepPrimAPI_MakePrism(feat_face2, gp_Vec(lenth2[0],lenth2[1],lenth2[2]))
        # feature_maker2 =  BRepPrimAPI_MakePrism(feat_face1, -gp_Vec(lenth[0],lenth[1],lenth[2]))
        # MYBody= BRepAlgoAPI_Fuse(feature_maker1.Shape(),feature_maker2.Shape())
        # shape1 = MYBody.Shape()

        # S1 = [shape1,feature_maker1,feature_maker2]
        ...
        return shape, new_labels,#S1

    def add_feature(self, bounds, dim, N_Choice,find_bounds=True):
        ...
        try:
            ...
            while try_cnt < len(self.bounds):
                if N_Choice is not  None:
                    N_BOUND = []
                    for s in range(len(self.bounds)):
                        if len(self.bounds[s]) == 5 and self.bounds[s][4][N_Choice[0]] == N_Choice[1]:
                            # print(bounds[s])
                            N_BOUND.append(self.bounds[s])
                        else:
                            continue                
                    bound_max2 = random.choice(N_BOUND)
                    depth2 = self._get_depth(bound_max2, triangles)

                    if depth2 <= 0 :
                        try_cnt += 1
                        continue

                bound_max1 = random.choice(self.bounds)
                depth1 = self._get_depth(bound_max1, triangles)
                ...
                if self.feat_type == 'through_hole' :
                    
                    feat_face,shape1, info= self._add_sketch(bound_max2)

                else:
                    feat_face = self._add_sketch(bound_max1)
        ...
        if self.feat_type == 'through_hole' :
            feat_dir = bound_max2[4]
            shape, labels = self._apply_hole_feature(self.shape, self.labels, self.feat_type, 
                                            feat_face,feat_dir * depth2, bound_max2)
            ...
            return shape, labels, self.bounds,info,shape1
        
        else:
            feat_dir = bound_max1[4]
            shape, labels = self._apply_feature(self.shape, self.labels, self.feat_type, 
                                            feat_face,feat_dir * depth1, bound_max1)
